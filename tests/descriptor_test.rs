...
#[cfg(test)]
mod tests {
  use cfd_rust::{Descriptor, Network, DescriptorKeyType, HashType};
  ...
  fn descriptor_pkh_test() {
    ...
    assert_eq!("pkh(02c6047f9441ed7d6d3045406e95c07cd85c778e4b8cef3ca7abac09b95c709ee5)#8fhd9pwu", descriptor.to_str());
    assert_eq!(&HashType::P2pkh, descriptor.get_hash_type());
    assert_eq!("1cMh228HTCiwS8ZsaakH8A8wze1JR5ZsP", descriptor.get_address().to_str());
    assert_eq!("OP_DUP OP_HASH160 06afd46bcdfd22ef94ac122aa11f241244a37ecc OP_EQUALVERIFY OP_CHECKSIG", descriptor.get_address().get_locking_script().to_asm());
  ...
  fn descriptor_wsh_multi_test() {
    ...
    assert_eq!("bc1qwu7hp9vckakyuw6htsy244qxtztrlyez4l7qlrpg68v6drgvj39qn4zazc", descriptor.get_address().to_str());
    assert_eq!("OP_0 773d709598b76c4e3b575c08aad40658963f9322affc0f8c28d1d9a68d0c944a",
    ...
    assert_eq!(3,
      descriptor.get_multisig_key_list().expect("Fail").len());
    assert_eq!("03a0434d9e47f3c86235477c7b1ae6ae5d3442d49b1943c2b752a68e2a47e247c7",
      descriptor.get_multisig_key_list().expect("Fail")[0].get_pubkey().to_hex());
    assert_eq!("03774ae7f858a9411e5ef4246b70c65aac5649980be5c17891bbec17895da008cb",
      descriptor.get_multisig_key_list().expect("Fail")[1].get_pubkey().to_hex());
  }
